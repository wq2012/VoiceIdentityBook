...
声纹验证只关心某个特定的说话人，可以被看作二值判别的问题。声纹辨认可能会有多个候选说话人。
...
我们希望将特征空间进行某种非线性变换。但通过核技巧，我们并不需要真的找出显式的非线性变换。
...
见第4.2.3节。在线应用需要用到流式信号处理——音频数据一边产生，一边作为语音处理引擎 的输入，并得到实时的处理结果。
...
声纹识别包含了录入与识别这两个阶段。一旦我们更换了新的声纹识别模型，已有的说话人模型将全部失效。

#### 练习 4.6 离线指标与在线代理指标的区别是什么？

离线指标：通过在专门的测试数据集上运行声纹识别系统，并根据测试数据中包含的真实说话人身份信息计算得到。

在线代理指标：用于真实使用场景，无法获取验证音频的真实说话人身份。只能间接、近似地反映声纹识别产品的性能。
...
包括：语音检测，语音分割与说话人转换检测，声纹嵌入码，聚类分析，以及二次分割。

#### 练习 5.2 除了声纹分割聚类，还有哪些多说话人识别的技术？这些技术与声纹分割聚类之间的主要区别分别是什么？

其他多说话人识别技术包括声源分离，连续声纹识别，说话人转换检测。
